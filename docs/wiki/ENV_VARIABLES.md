...
- `MISSION_CATALOG_SYNC_CRON_HOUR` / `MISSION_CATALOG_STARTUP_MAX_AGE_HOURS` - Catalog reconciliation schedule (UTC hour, default 6) and startup freshness window (default 24h). Prefer interval below when set.
- `MISSION_CATALOG_SYNC_INTERVAL_MINUTES` - When > 0 (default **30**), leader catalog sync runs on this interval instead of daily cron. Set `0` to keep daily cron only.
...
- `MISSION_CATALOG_ENROLLMENT_SHADOW` - When `true`, ST ACTIVE would-enroll / would-complete transitions are logged without writing `CONTINUOUS`; default **`false`**.
- `MISSION_CATALOG_WG_SYNC_FROM_CATALOG` - When `true`, WG `sync_all_realtime_missions` reads keys via catalog enablement API (env override when non-empty; enrolled ACTIVE∧CONTINUOUS when empty); default **`false`**
- `MISSION_CATALOG_SLOCUM_WARM_FROM_CATALOG` - When `true`, Slocum warm reads keys via catalog enablement; default **`false`** in code. Local/prod soaks run with WG sync / AUTO_APPLY / Slocum warm **true**. Ops: [mission catalog cutover](./how-tos/mission_catalog_cutover.md)
- `MISSION_CATALOG_PUBLIC_MAP_FROM_CATALOG` - When `true`, public login map allowlist via catalog enablement ∩ `public_map_enabled`; default **`false`**

Enablement membership (all `MISSION_CATALOG_*_FROM_CATALOG` consumers): non-empty `ACTIVE_REALTIME_MISSIONS` / `ACTIVE_SLOCUM_DATASETS` / historical lists are an emergency exact-string override; empty lists derive from catalog **ACTIVE ∧ CONTINUOUS** with live-row gates (WG: realtime WGMS source + linked overview; Slocum: linked `is_active` deployment). ST auto-enrolls allowlisted ACTIVE missions (`start_time` reached, no `end_time`) unless `enrollment_override=forced_off`. Numbered ST rows without start stay **PLANNED** (workspace only).
