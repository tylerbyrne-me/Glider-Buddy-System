...
- Checklist ERDDAP wishlist includes digifin leak detect and thruster vars; preprocess via checklist rename/std columns
- Plot modal (`web/static/js/slocum_checklist_form.js`): depth on left (`y`); value series on `y2`; digifin / thruster % on `y3` when needed
- Plottable autofill rows include vacuum/attitude/buoyancy (legacy), plus **water depth**, **BMS currents**, **leak channels** (main/forward/science/digifin), and combined **thruster** (`m_thruster_power` / `c_thruster_on`)
...
- **`app/routers/slocum.py`**: Mission dashboard chart/CSV/profile APIs.
- **`app/routers/slocum_checklists.py`**: Daily checklist template/submit/series/compare APIs and form page.
- **`app/core/slocum_checklist_autofill.py`** / **`slocum_checklist_compare.py`**: Autofill, Plot-it payloads, form-to-form diff.
...
- **`web/static/js/slocum_dashboard.js`**: Declarative sensor-card chart configs, checklist tab, compare entry.
