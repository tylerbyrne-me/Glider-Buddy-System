...
| DMON | `sci_dmon_msg_byte_count` over time; SFMC `from-glider` `*.asc` files (last 48h) with >16h gap highlight and **Thruster since prev** Yes/No (plus event count, on-time, and depth range when Yes) from dashboard `m_thruster_power` / `c_thruster_on` over each `[prev_mtime, this_mtime)` interval — surface bursts at ≤3 m ignored via nearest `m_depth` within 60 s; expand each ASC row for UTC HH:MM event timing; left-nav green/red status dot (same style as Wave Glider Waves ESS); **Robots4Whales daily analyst review** (last 48h table + full-history collapse) when `robots4whales_url` is set on the deployment |
...
| `GET /api/slocum/sfmc/dmon-asc-files/{dataset_id}` | Cached SFMC `from-glider` `*.asc` listing + gap flags; enriches each file with subsurface `thruster_since_prev` (+ event count, minutes, depth range, and `thruster_events` timing) from the 48h dashboard mirror (DMON card; ≤3 m surface bursts excluded). |
