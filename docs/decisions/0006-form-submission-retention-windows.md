...
# Title: Form list APIs use summary payloads and 30-day interactive windows
...
2. **Mission-scoped PIC and Slocum checklist lists default to 30 days.** Expand via `days=0` plus pagination. Pilot global forms stay 72h; recent PIC stays 24h; my PIC defaults to 90 days.
...
- **Keep full blobs in list responses but paginate only** — rejected; even 30 days × full JSON remains unnecessarily heavy for a metadata table.
...
- Mission PIC and Slocum checklist tabs show a “last 30 days” affordance and a control to load older pages.
