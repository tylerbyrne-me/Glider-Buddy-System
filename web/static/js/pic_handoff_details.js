 ...
 * @param {Object|null} currentUser - Current logged-in user ({ username, role }) for edit link gating
 * @returns {string} HTML string for modal body
 */
export function renderPicHandoffDetails(form, changedItemIds = [], currentUser = null) {
    ...
    if (form.edited_by_username && form.last_edited_timestamp) {
        const editedTimestampStr = String(form.last_edited_timestamp).endsWith('Z')
            ? String(form.last_edited_timestamp)
            : `${form.last_edited_timestamp}Z`;
        const editedDate = new Date(editedTimestampStr);
        const formattedEditedTime = Number.isNaN(editedDate.getTime())
            ? 'N/A'
            : editedDate.toLocaleString('en-GB', { timeZone: 'UTC', dateStyle: 'medium', timeStyle: 'medium', hour12: false }) + ' UTC';
        html += `<p class="mb-2 text-muted"><strong>Edited by:</strong> ${escapeHtml(form.edited_by_username)} at ${escapeHtml(formattedEditedTime)}</p>`;
    }
    const canEdit = currentUser && form.id && form.form_type === 'pic_handoff_checklist' && (
        currentUser.role === 'admin' || currentUser.username === form.submitted_by_username
    );
    if (canEdit) {
        const editUrl = `/mission/${encodeURIComponent(form.mission_id)}/form/pic_handoff_checklist.html?edit=${encodeURIComponent(form.id)}`;
        html += `<p class="mb-2"><a class="btn btn-sm btn-outline-primary" href="${editUrl}">Edit submission</a></p>`;
    }
